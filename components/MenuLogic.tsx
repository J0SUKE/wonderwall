...
export default function MenuLogic() {
  ...
  function openMenu() {
    ...
    openTimeline.fromTo(
      (menuRef.current.nextSibling as HTMLDivElement).childNodes[0],
      {
        height: "0vh",
      },
      {
    ...
    openTimeline.fromTo(
      (menuRef.current.nextSibling as HTMLDivElement).childNodes[1],
      {
        height: "0vh",
      },
      {
    ...
    openTimeline.fromTo(
      menuRef.current.querySelector(".close-btn"),
      { scale: 0 },
      {
        scale: 1,
      },
  ...
  useEffect(() => {
    window.addEventListener("resize", () => {
      if (!menuIsOpen.current) {
        ...
        gsap.to(menuRef.current.nextSibling, {
          duration: 0,
          x: window.innerWidth / 2,
        });
      }
  ...
  return (
    <>
      ...
      />
      <Menu menuRef={menuRef} CloseMenu={CloseMenu} />
    </>
  );
}
