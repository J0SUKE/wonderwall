...
export default function BottomSlider({
  ...
  BlueZone: React.RefObject<HTMLDivElement>;
}) {
  const ButtonsContainer = useRef<HTMLDivElement>(null);
  const SliderPrevButton = useRef<HTMLButtonElement>(
    null
  ) as React.MutableRefObject<HTMLButtonElement>;
  const SliderNextButton = useRef<HTMLButtonElement>(
    null
  ) as React.MutableRefObject<HTMLButtonElement>;
  ...
  return (
    <div className="pb-[3rem]">
      <p
        className={`pl-[5%] transition-[color] duration-300 ${
      ...
      <div className="px-[5%] flex justify-between">
        <h2
          className={`text-[clamp(2.5rem,5vw,5vmax)] transition-[color] duration-300  uppercase ${
      ...
      <div className="hidden md:block mt-[1.3rem]">
        ...
        >
          ...
          />
          <SliderItem
            image="/images/Ambre.jpg"
            title="Ambre"
            desc="310 PLN per m"
          />
          <SliderItem
            image="/images/Airon-1.jpg"
            title="Ambre"
            desc="310 PLN per m"
          />
          <SliderItem
            image="/images/Volare.jpg"
            title="Volare"
            desc="310 PLN per m"
          />
          <SliderItem
            image="/images/Fereldan.jpg"
            title="Fereldan"
            desc="310 PLN per m"
          />
          <SliderItem
      ...
      />
      <HoverButton
        link="https://wonderwall-studio.pl/kategoria-produktu/tapety/?lang=en"
        text="See more"
        icon="points"
        style={`ml-[5%] mt-[3rem] border ${
          white ? "border-[#cdcdcd]" : "border-[#cdcdcd65]"
        } transition-[border] duration-300 hover:border-[transparent]`}
        white={white}
      />
    </div>
  );
...
function MobileCaroussel({
  ButtonsContainer,
  SliderPrevButton,
  SliderNextButton,
}: {
  ButtonsContainer: React.RefObject<HTMLDivElement>;
...
function SliderItem({
  ...
  return (
    <div className="w-[70vw] md:w-[40vmax] pl-[5vmax] overflow-hidden">
      ...
      <h3
        className={`mt-[1.4rem] text-[1.5rem] text-left ${
          white ? "text-[black]" : "text-[white]"
        }`}
      >
        {title}
      ...
      <p
        className={`text-[.9rem] mt-[.4rem] text-left 
                ${
                  white
                    ? "text-[#202020] opacity-[.5]"
                    : "text-[#dfdfdf] opacity-[1]"
                }`}
      >
        {desc}
      </p>
    ...
    <>
      {ButtonsContainer.current
        ? ReactDOM.createPortal(
            <>
              <button
                className={`hidden md:flex h-[3.7rem] w-[3.7rem] transition-all duration-300 rounded-[50%] border ${
                  white
                    ? "border-[#a9a9a96c] slider_btn slider_btn-w"
                    : "border-[#e5e5e570] slider_btn"
                } overflow-hidden  justify-center items-center relative`}
              ...
              >
                ...
                />
                <div
                  className={`absolute z-[1] h-[100%] w-[100%] ${
                    white ? "bg-[#82969C]" : "bg-[white]"
                  } rounded-t-[50%]`}
                ></div>
              </button>
            </>,
