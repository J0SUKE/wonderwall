...
export default function Hero() {
  ...
  const heroCtaButton = useRef<HTMLButtonElement>(
    null
    ...
    <div className="h-[100vh] relative z-[1] overflow-hidden" data-speed="1">
      ...
      <div className="absolute z-3 w-[100%] h-[100%] top-0 left-0 pl-[2%] lg:pl-[20%] pr-[2%] flex flex-col justify-end items-start">
        <div className="w-[100%] flex justify-between items-end">
            ...
            <div className="h-[clamp(2.6rem,6.8vw,6.8vmax)]">
              <div className="uppercase flex gap-[.15rem] overflow-hidden">
                {"wallpapers".split("").map((letter: string) => (
                  <span className="block" key={Math.random() * 100}>
                    {letter}
                  </span>
                ))}
              </div>
              ...
              <div className="uppercase flex gap-[2rem]">
                ...
                <div className="flex gap-[.1rem] text-[clamp(2.3rem,6.5vw,6.5vmax)] h-[clamp(2.6rem,6.8vw,6.8vmax)] overflow-hidden">
                  {"inspire".split("").map((letter: string) => (
                    <span className="block noto" key={Math.random() * 100}>
                      {letter}
                    </span>
                  ))}
                </div>
          ...
          <SliderButtons
            style="pb-[3rem] hidden sm:flex gap-[1rem]"
        ...
        <div className="w-[100%] pb-[clamp(2rem,3vw,3vmax)] pt-[6rem] lg:pt-[clamp(3rem,6vw,6vmax)] flex justify-between items-center">
          ...
          <p className="hidden sm:flex text-[white] text-[clamp(2rem,5vw,5vmax)] items-end gap-[clamp(1rem,1vw,1vmax)]">
            <span
              ref={indexIndicator}
          ...
          <SliderButtons
            style="flex gap-[1rem] sm:hidden sm:gap-[0rem]"
            sliderIndex={sliderIndex}
            sliderRef={sliderRef}
            heroTitleRef={heroTitleRef}
            heroCtaButton={heroCtaButton}
...
function SliderButtons({
  ...
  sliderIndex: React.MutableRefObject<number>;
  heroTitleRef: React.RefObject<HTMLHeadingElement>;
  heroCtaButton: React.MutableRefObject<HTMLButtonElement>;
  indexIndicator: React.MutableRefObject<HTMLSpanElement>;
}) {
  ...
  function NextSlide() {
    ...
    nextTimeline.fromTo(
      slides[sliderIndex.current],
      { scale: 1 },
      { scale: 1.1, duration: 1 }
    );

    const nextIndex =
      sliderIndex.current == max - 1 ? 0 : sliderIndex.current + 1;
    ...
    nextTimeline.fromTo(
      [...heroTitleRef.current.querySelectorAll("span")].reverse(),
      {
        yPercent: 0,
      },
      {
    ...
    nextTimeline.to(
      heroCtaButton.current,
      {
        ...
        onComplete: () => {
          ...
          if (sliderIndex.current == max - 1) sliderIndex.current = 0;
  ...
  function PreviousSlide() {
    ...
    const prevIndex =
      sliderIndex.current == 0 ? max - 1 : sliderIndex.current - 1;
    ...
    prevTimeline.to(
      heroCtaButton.current,
      {
        ...
        onComplete: () => {
          ...
          if (sliderIndex.current == 0) sliderIndex.current = max - 1;
          else sliderIndex.current--;
        },
      },
      "-=0.3"
